...
login("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")
...
def topk_sampling(model, prompt, max_len, topk, tokenizer, temperature=1.0):
    model.eval()
    ...
    with torch.no_grad():
        for _ in range(max_len - input_ids.shape[1]):
            ...
            if next_token.item() == tokenizer.eos_token_id:
                break
    text = " ".join([tokenizer.id_to_str[idx.item()] for idx in generated[0]])
    return text
...
if __name__ == '__main__':
    ...
    # print(topk_sampling(model, ["Is Stockholm the capital of Sweden? Answer yes or no. The answer is"], max_len=50, topk=3, tokenizer=tokenizer, temperature=2.0))
